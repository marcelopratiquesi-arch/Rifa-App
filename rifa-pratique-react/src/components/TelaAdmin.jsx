...
import {
  ...
  WhatsappLogo, CheckCircle, XCircle, Clock, FunnelSimple
...
const TOTAL_COTAS    = 1000; // ✅ Total da rifa para o termômetro
...
export default function TelaAdmin() {
  ...
  // ✅ MELHORIA 2 — Filtro por vendedor
  ...
  // ✅ MELHORIA 3 — Parabenizar vencedor via WhatsApp
  const parabenizarVencedor = (ganhador, lugar) => {
    const tel  = ganhador.pedido.tel.replace(/\D/g, '');
    const nome = ganhador.pedido.nome.split(' ')[0];
    const premio = lugar === 1 ? 'Cesta Ouro (1 lugar)' : 'Cesta Prata (2 lugar)';
    const msgs =
      "Ola *" + nome + "*!\n\n" +
      "Aqui e o Marcelo da Rifa Pratique.\n\n" +
      "Tenho uma OTIMA noticia para voce:\n" +
      "O numero *" + ganhador.numero + "* foi sorteado e voce GANHOU a *" + premio + "*!\n\n" +
      "Por favor, me confirme seu endereco completo para combinarmos a entrega do seu premio.\n\n" +
      "Parabens!";
    window.open(`https://wa.me/55${tel}?text=${encodeURIComponent(msgs)}`, '_blank');
  };
  ...
  // ✅ MELHORIA 1 — Dados para o termômetro
  const totalCotasVendidas   = pedidosAprovados.reduce((s, p) => s + (p.nums || []).length, 0);
  const totalCotasReservadas = pedidos.filter(p => p.status === 'pendente').reduce((s, p) => s + (p.nums || []).length, 0);
  ...
  // ✅ MELHORIA 2 — pedidosFiltrados agora inclui filtro por vendedor
  const pedidosFiltrados = pedidos.filter((p) => {
    ...
    // A linha nova fica assim:
const matchVendedor = !filtroVendedor || 
                      (filtroVendedor === 'DIRETO' ? !p.vendedor : p.vendedor === filtroVendedor);
    return matchStatus && matchBusca && matchVendedor;
  });

  // Badge de contagem para as abas
    ...
    <div className="animate-fade-in max-w-5xl mx-auto mt-4 pb-16">
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              ...
              {vendedores.length > 0 && (
                <div className="relative">
                  <FunnelSimple size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-400" />
                  <select
                    value={filtroVendedor}
                    onChange={(e) => setFiltroVendedor(e.target.value)}
                    className="pl-8 pr-4 bg-zinc-50 dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-700 rounded-lg py-2 text-zinc-900 dark:text-white text-sm focus:border-orange-500 focus:outline-none appearance-none w-full sm:w-auto"
                  >
            ...
            {pedidosFiltrados.length === 0 ? (
              ...
              pedidosFiltrados.map((p) => (
                <div key={p.id} className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 overflow-hidden hover:border-zinc-300 dark:hover:border-zinc-600 transition-colors">
                  <div className={`h-1 w-full ${p.status === 'pago' ? 'bg-green-500' : p.status === 'pendente' ? 'bg-yellow-500' : 'bg-zinc-400'}`} />
                  <div className="p-4">
                    ...
                    <div className="flex flex-wrap gap-1 mb-3">
                      {(p.nums || []).map((n) => (
                        <span key={n} className={`text-xs font-mono font-bold px-1.5 py-0.5 rounded ${
                          p.status === 'pago' ? 'bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-300' :
                          p.status === 'pendente' ? 'bg-yellow-100 dark:bg-yellow-900 text-yellow-700 dark:text-yellow-300' :
                    ...
                    <div className="flex flex-wrap gap-2">
                      {p.status === 'pendente' && (
                        <button onClick={() => cobrarWhatsApp(p)} className="flex items-center gap-1.5 bg-[#25D366] hover:bg-[#1ebe57] text-white text-xs font-bold px-3 py-2 rounded-lg transition-colors">
                          <WhatsappLogo size={14} weight="fill" /> Cobrar
                        </button>
                      )}
                      {p.status === 'pendente' && (
                        <button onClick={() => aprovarPedido(p.id)} className="flex items-center gap-1.5 bg-green-600 hover:bg-green-500 text-white text-xs font-bold px-3 py-2 rounded-lg transition-colors">
                          <CheckCircle size={14} weight="fill" /> Confirmar PIX
                        </button>
                      )}
                      {p.status === 'pendente' && (
                        <button onClick={() => expirarPedido(p.id)} className="flex items-center gap-1.5 bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-700 text-red-500 text-xs font-bold px-3 py-2 rounded-lg transition-colors border border-zinc-200 dark:border-zinc-700">
            ...
            {ganhadores && !sorteando && (
              <div className="mt-10 grid grid-cols-1 md:grid-cols-2 gap-6 text-left animate-fade-in">
                ...
                <div className="bg-gradient-to-br from-yellow-100 to-yellow-50 dark:from-yellow-900/30 dark:to-yellow-900/10 border-2 border-yellow-400 dark:border-yellow-600 rounded-xl p-6 relative overflow-hidden">
                  ...
                  {/* ✅ MELHORIA 3 — Botão Parabenizar */}
                  <button
                    onClick={() => parabenizarVencedor(ganhadores.primeiro, 1)}
                    className="w-full flex items-center justify-center gap-2 bg-[#25D366] hover:bg-[#1ebe57] text-white font-bold py-2.5 rounded-lg transition-colors text-sm"
                  >
      ...
      {abaAtiva === 'relatorios' && (
        <div className="animate-fade-in space-y-6">
          ...
          {/* ✅ MELHORIA 1 — Termômetro da Rifa */}
          <div className="bg-white dark:bg-zinc-900 p-6 rounded-xl border border-zinc-200 dark:border-zinc-800">
            <div className="flex justify-between items-center mb-3">
              <h3 className="font-bold text-zinc-900 dark:text-white flex items-center gap-2">
                🔥 Termômetro da Rifa
              </h3>
              <span className="text-sm font-black text-orange-500">{pctTotal}% ocupado</span>
            </div>

            {/* Barra principal */}
            <div className="w-full bg-zinc-100 dark:bg-zinc-800 rounded-full h-5 overflow-hidden mb-2">
              {/* Pagas (verde) + Reservadas (amarelo) empilhadas */}
            ...
            <div className="flex flex-wrap gap-4 text-xs text-zinc-500 mt-3">
              ...
              <span className="flex items-center gap-1.5">
                <span className="w-3 h-3 rounded-sm bg-zinc-200 dark:bg-zinc-700 inline-block" />
              ...
              {['Santa Inês 1', 'Santa Inês 2'].map((unidade) => {
                ...
                return (
                  <div key={unidade} className={`p-4 rounded-xl border-2 text-center transition-all ${liderando ? 'border-orange-500 bg-orange-50 dark:bg-orange-900/10 scale-105 shadow-lg' : 'border-zinc-200 dark:border-zinc-800'}`}>
                    ...
                    <p className="text-2xl font-black text-green-600 dark:text-green-400 mt-2">R$ {fmtValor(dados?.valor || 0)}</p>
                    <p className="text-sm text-zinc-500">{dados?.cotas || 0} cotas</p>
                    {liderando && <span className="inline-block mt-3 bg-orange-500 text-white text-xs font-bold px-3 py-1 rounded-full animate-bounce">🏆 LIDERANDO</span>}
                  </div>
                );
              })}
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="bg-white dark:bg-zinc-900 p-6 rounded-xl border border-zinc-200 dark:border-zinc-800 flex flex-col justify-center gap-3">
              <button onClick={exportarExcel} className="w-full bg-[#107C41] hover:bg-[#0d6535] text-white font-bold py-2.5 rounded-lg flex justify-center items-center gap-2 transition-colors">
                <DownloadSimple size={20} /> Baixar Excel
              </button>
              <button onClick={exportarPDF} className="w-full bg-[#E53E3E] hover:bg-[#c53030] text-white font-bold py-2.5 rounded-lg flex justify-center items-center gap-2 transition-colors">
              ...
              <table className="w-full text-left text-sm">
                <thead>
                  <tr className="border-b border-zinc-200 dark:border-zinc-800 text-zinc-500">
                    <th className="pb-3">Pos</th><th className="pb-3">Vendedor</th>
                    <th className="pb-3">Unidade</th><th className="pb-3">Cotas</th><th className="pb-3">Valor</th>
                  </tr>
                </thead>
                <tbody>
                  {rankVend.map((v, i) => (
        ...
        <div className="animate-fade-in grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-white dark:bg-zinc-900 p-6 rounded-xl border border-zinc-200 dark:border-zinc-800 h-fit">
            ...
            <form onSubmit={handleAdicionarVendedor} className="flex flex-col gap-3">
              <input type="text" placeholder="Nome do Vendedor" value={novoVendedor} onChange={(e) => setNovoVendedor(e.target.value)}
                className="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-700 rounded-lg p-3 text-zinc-900 dark:text-white focus:border-orange-500 focus:outline-none" />
              <div className="flex gap-2">
                ...
                <button type="submit" className="bg-orange-600 hover:bg-orange-500 text-white font-bold px-6 py-3 rounded-lg transition-colors">Salvar</button>
              </div>
            </form>
          </div>
          <div className="bg-white dark:bg-zinc-900 p-6 rounded-xl border border-zinc-200 dark:border-zinc-800">
            <h3 className="font-bold mb-4 text-zinc-900 dark:text-white">Vendedores Cadastrados ({vendedores.length})</h3>
            <div className="space-y-2 max-h-72 overflow-y-auto pr-1 scrollbar-thin">
              {vendedores.length === 0 && <p className="text-sm text-zinc-500">Nenhum vendedor cadastrado ainda.</p>}
              {vendedores.map((v) => (
